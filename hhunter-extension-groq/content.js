;(function () {
  ...
  async function runOnce() {
    if (detectCaptcha()) {
      await sendBg({ type: 'report', kind: 'error', last: { level: 'WARNING', message: 'Капча на hh.ru — остановите и решите вручную' } })
      return
    }
    if (pageKind() !== 'vacancy') {
      await sendBg({ type: 'report', last: { level: 'INFO', message: 'Откройте страницу вакансии hh.ru/vacancy/{id}' } })
      return
    }
    ...
    const apiBase = (await chrome.storage.local.get('hhunter_api_base')).hhunter_api_base || 'http://localhost:8000'
    const token = (await chrome.storage.local.get('hhunter_token')).hhunter_token || ''
    if (!token) {
      await sendBg({ type: 'report', kind: 'error', last: { level: 'ERROR', message: 'Нет JWT токена. Откройте сайт HHunter и синхронизируйте токен.' } })
      return
    }

    const res = await fetch(`${apiBase.replace(/\\/$/, '')}/extension/generate-letter`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({ vacancy_title: payload.vacancy_title, vacancy_description: payload.vacancy_description, company_name: payload.company_name }),
    })
    const data = await res.json().catch(() => ({}))
    if (!res.ok) {
      const msg = data?.detail || res.statusText
      await sendBg({ type: 'report', kind: 'error', last: { level: 'ERROR', message: `generate-letter: ${msg}` } })
      return
    }

    // 2) вставить в форму отклика (если найдено)
    ...
    // 3) сохранить запись на сервер
    await fetch(`${apiBase.replace(/\\/$/, '')}/extension/save-application`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify({
    ...
    await sendBg({ type: 'report', kind: 'sent', last: { level: 'INFO', message: `Готово: ${payload.vacancy_title}` } })
  }

  chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
    if (msg.type === 'hhunter_run_once') {
